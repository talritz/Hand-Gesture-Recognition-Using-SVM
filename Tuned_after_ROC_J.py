...
def main():
    ...
    for kernel, param in model_list.items():
        if kernel == 'linear':
            base_model = LinearSVC(class_weight='balanced', random_state=42, **param, max_iter=10000)
        else:
            base_model = SVC(kernel=kernel, class_weight='balanced', random_state=42, **param)
